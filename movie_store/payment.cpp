...
void addCreditCard(customer customers[], int customers_count, std::string& id, bool isDateChanged) // done
{
    ...
    if (isDateChanged) ///note
    {
        system_date = new_date; // this is a global variable not a copy
...
double amount2pay(movie& mov, bool isDateChanged)
{
    int fee_days = validateDue(mov, isDateChanged);
...
bool pay(double& cashRegister, customer customers[], int customers_count, 
         std::string& id, movie& movie, bool isDateChanged) 
{
    int customerIndex = getCustomerIndex(customers, customers_count, id);
    int ans;
    bool paid = false;
    ...
    double amount = amount2pay(movie, isDateChanged); //due status is set now
        ...
        case 2:
        {
            ...
            else
            {
                ...
                if (yes_no())
                {
                    ...
                    if (customers[customerIndex].creditcard.cardNumber.empty()) break;
